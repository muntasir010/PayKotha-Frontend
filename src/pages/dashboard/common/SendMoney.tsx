...
const SendMoney = () => {
  ...
  const [sendMoney, { isLoading, isError, isSuccess }] = useSendMoneyMutation();
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        const res = await axiosInstance.get(`/users/search?name=${recipientName}`);
  ...
  return (
    <div className="mx-auto shadow-md rounded-2xl p-6 mt-10 max-w-md">
      ...
      <div className="mt-4 text-center">
        ...
        {isError && (
          <p className="text-red-600 font-medium">Failed To Send Money</p>
        )}
      </div>
    </div>
  );
